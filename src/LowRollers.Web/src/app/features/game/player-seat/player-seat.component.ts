...
@Component({
          ...
          <div class="avatar-wrapper" [class.active]="p.isCurrentTurn">
            ...
            >
              ...
              @if (p.isCurrentTurn && p.remainingTime !== undefined) {
                ...
                <span class="timer-text" [class.warning]="timerPercentage() < 50" [class.danger]="timerPercentage() < 20">
            ...
            <div class="chip-stack">
              @for (stack of betChipStacks(); track stack.color) {
                <div class="chip-column">
                  @for (i of getChipArray(stack.count); track i) {
                    <div class="chip">
                      <svg viewBox="0 0 100 100"><use [attr.href]="'#chip' + capitalizeFirst(stack.color)" /></svg>
                    </div>
                  }
                </div>
              }
  ...
  styles: [`
    ...
    .player-avatar.active {
      ...
      box-shadow: 0 0 0 3px var(--border-active), 0 6px 20px rgba(0, 0, 0, 0.4);
    }
    ...
    .timer-bar-progress {
      ...
      background: #22c55e;
      border-radius: 3px;
      transition: width var(--animation-very-slow) linear, background var(--animation-normal);
    }
...
export class PlayerSeatComponent {
  ...
  /** Create array for ngFor iteration */
  getChipArray(count: number): number[] {
    return Array.from({ length: count }, (_, i) => i);
  }

  /** Capitalize first letter for chip symbol reference */
