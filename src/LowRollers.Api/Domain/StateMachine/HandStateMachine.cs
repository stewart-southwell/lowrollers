...
/// </summary>
public sealed class HandStateMachine
{
    private readonly ILogger<HandStateMachine> _logger;
    private readonly IReadOnlyDictionary<HandPhase, IHandPhaseHandler> _handlers;
    private readonly List<HandStateTransition> _transitionHistory = [];

    // Valid state transitions: current phase -> allowed next phases
    private static readonly IReadOnlyDictionary<HandPhase, HashSet<HandPhase>> ValidTransitions =
        new Dictionary<HandPhase, HashSet<HandPhase>>
        {
        ...
        PhaseTransitionContext? context = null)
    {
        ...
        _logger.LogDebug(
            "Attempting transition: {HandId} from {CurrentPhase} to {TargetPhase} via {Trigger}",
            hand.Id, currentPhase, targetPhase, trigger);
        ...
        if (!IsTransitionValid(currentPhase, targetPhase))
        {
            ...
            _logger.LogWarning(
                "Transition denied for {HandId}: {Error}",
                hand.Id, error);
        ...
        if (_handlers.TryGetValue(currentPhase, out var currentHandler))
        {
            ...
            if (!validation.IsValid)
            {
                ...
                _logger.LogWarning(
                    "Transition validation failed for {HandId}: {Errors}",
                    hand.Id, errors);
        ...
        if (_handlers.TryGetValue(currentPhase, out currentHandler))
        {
            try
            {
                ...
                _logger.LogTrace("Exited phase {Phase} for hand {HandId}", currentPhase, hand.Id);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error during exit from phase {Phase} for hand {HandId}", currentPhase, hand.Id);
        ...
        _logger.LogInformation(
            "Hand {HandId} transitioned from {FromPhase} to {ToPhase} via {Trigger}",
            hand.Id, previousPhase, targetPhase, trigger);
        ...
        if (_handlers.TryGetValue(targetPhase, out var newHandler))
        {
            try
            {
                ...
                _logger.LogTrace("Entered phase {Phase} for hand {HandId}", targetPhase, hand.Id);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error during entry to phase {Phase} for hand {HandId}", targetPhase, hand.Id);
